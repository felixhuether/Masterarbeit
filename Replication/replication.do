...
- IHS-transformation (see section 3) largely depends on the magnitude of the values of the transformed variable (Aïhounton and Henningsen 2019)
...
predict res_resources, resid
tsline res_resources, name(resources_res)
tsline ihs_e_total_resources_pc_c, name(resources)
graph combine resources resources_res
...
drop res_lib res_gdp res_exports res_imports res_exp_us res_imp_us res_resources res_v2cacamps
graph drop _all
...
** MI is designed for normally distributed variables (Lee and Carlin 2010) which is taken care of by the IHS-transformation.
...
Aïhounton, Ghislain B. D., and Arne Henningsen. 2019. "Units of Measurement and the Inverse Hyperbolic Sine Transformation." IFRO Working Paper 2019/10.
